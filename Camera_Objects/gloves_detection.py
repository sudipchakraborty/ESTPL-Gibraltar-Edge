"""Protective-glove detection and alert generation."""
...
class GlovesDetectionMonitor:
    """Detect worn gloves and queue a debounced alert."""
    ...
    def __init__(self, config: dict, repository_dir: Path) -> None:
        ...
        if not model_path.is_file():
            raise FileNotFoundError(f"Gloves detection model not found: {model_path}")
        ...
        self.gloves_classes = {
            ...
            for name in config.get("gloves_class_names", ["gloves", "glove"])
        }
        ...
        if not available_names.intersection(self.gloves_classes):
            raise ValueError(
                "The PPE model has none of the configured glove classes. "
                f"Model classes: {sorted(available_names)}; "
                f"configured classes: {sorted(self.gloves_classes)}"
            )
    ...
    def process(self, frame):
        """Annotate gloves and queue an alert after consecutive detections."""
        ...
        for box in result.boxes:
            ...
            if class_name.casefold() not in self.gloves_classes:
                continue
            detection = {
            ...
            cv2.putText(
                frame,
                f"Gloves {detection['confidence']:.2f}",
                (x1, max(22, y1 - 8)),
        ...
        if detections:
            ...
            if (
                ...
                print(
                    f"[GLOVES ALERT] Gloves detected: count={len(detections)}",
                    flush=True,
                )
        else:
            self._detection_frames = 0
    ...
    def pop_alert(self) -> dict[str, Any] | None:
        """Return and clear the latest queued gloves alert."""
