"""Face-mask detection and alert generation."""
...
class MaskDetectionMonitor:
    """Detect configured mask classes and queue an alert for mask wearers."""
    ...
    def __init__(self, config: dict, repository_dir: Path) -> None:
        ...
        if not model_path.is_file():
            raise FileNotFoundError(
                f"Mask detection model not found: {model_path}. "
                "Provide a YOLO model trained to detect face masks."
            )

        self.model = YOLO(str(model_path))
        self.device = config.get("device", "cpu")
        self.confidence = float(config.get("confidence_threshold", 0.5))
        self.mask_classes = {
            str(name).strip().casefold()
            for name in config.get(
                "mask_class_names", ["mask", "with_mask", "masked"]
            )
        }
        ...
        if not available_names.intersection(self.mask_classes):
            raise ValueError(
                "The mask model has none of the configured mask classes. "
                f"Model classes: {sorted(available_names)}; "
                f"configured classes: {sorted(self.mask_classes)}"
            )
    ...
    def process(self, frame):
        """Annotate mask detections and queue a debounced mask-worn alert."""
        ...
        for box in result.boxes:
            ...
            if class_name.casefold() not in self.mask_classes:
                continue
            detection = {
            ...
            cv2.putText(
                frame,
                f"Mask {detection['confidence']:.2f}",
                (x1, max(22, y1 - 8)),
        ...
        if detections:
            ...
            if (
                ...
                print(
                    f"[MASK ALERT] Mask wearer detected: count={len(detections)}",
                    flush=True,
                )
        else:
            self._detection_frames = 0
    ...
    def pop_alert(self) -> dict[str, Any] | None:
        """Return and clear the latest queued mask alert."""
