"""Gun detection and alert generation."""
...
class GunDetectionMonitor:
    """Detect visible guns and queue a debounced alert."""
    ...
    def __init__(self, config: dict, repository_dir: Path) -> None:
        ...
        if not model_path.is_file():
            raise FileNotFoundError(f"Gun detection model not found: {model_path}")

        self.model = YOLO(str(model_path))
        self.device = config.get("device", "cpu")
        self.confidence = float(config.get("confidence_threshold", 0.4))
        self.gun_classes = {
            str(name).strip().casefold()
            for name in config.get("gun_class_names", ["gun", "weapon"])
        }
        ...
        if not available_names.intersection(self.gun_classes):
            raise ValueError(
                "The weapon model has none of the configured gun classes. "
                f"Model classes: {sorted(available_names)}; "
                f"configured classes: {sorted(self.gun_classes)}"
            )
    ...
    def process(self, frame):
        """Annotate guns and queue an alert after consecutive detections."""
        ...
        for box in result.boxes:
            ...
            if class_name.casefold() not in self.gun_classes:
                continue
            detection = {
        ...
        if detections:
            ...
            if (
                ...
                print(
                    f"[GUN ALERT] Gun detected: count={len(detections)}",
                    flush=True,
                )
        else:
            self._detection_frames = 0
    ...
    def pop_alert(self) -> dict[str, Any] | None:
        """Return and clear the latest queued gun alert."""
