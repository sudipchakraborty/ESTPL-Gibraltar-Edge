...
def run_gui(args: argparse.Namespace) -> int:
    ...
    class CameraPanel(QFrame):
        match_detected = Signal(str, float, str, object, bytes)
    ...
    class GibraltarWindow(QMainWindow):
        def __init__(self) -> None:
            ...
            self._rtsp_transport = str(
                project_config.get("rtsp_transport", "udp")
            ).lower()
        ...
        def _update(self) -> None:
            ...
            for camera, panel, input_queue in zip(
                ...
                if ok:
                    ...
                    panel.show_jpeg(encoded, display=True, evaluate=False)
                    self._webrtc_service.publish(panel._camera_name, frame)
